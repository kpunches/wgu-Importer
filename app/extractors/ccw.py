...
EVIDENCE_MARKERS = [
    "Potential Evidence of Competency at Specified Level:",
    "Evidence of Competency at Specified Level:",
    "Potential Evidence:",
...
def _parse_modality_and_rationale(text):
    """
    Split assessment block into (modality, rationale, evidence).
    Handles: 'Modality: OA Rationale: ... Potential Evidence: ...'
    Returns (modality, rationale, evidence).
    """
    modality = ""
    evidence = ""

    m = re.match(r"Modality:\s*(\S+)\s*(?:Rationale:)?\s*(.*)", text, re.DOTALL | re.IGNORECASE)
    if m:
    ...
    return modality, rationale, evidence
        ...
        for label, field in TABLE_LABELS.items():
            if label in header:
                ...
                if field == "assessment_block":
                    modality, rationale, evidence = _parse_modality_and_rationale(content)
                    course_level["modality"] = modality
                    course_level["assessment_rationale"] = rationale
                    if evidence:
                        course_level["evidence"] = evidence
                else:
            ...
            for key, field in ROW_LABELS.items():
                if key in label:
                    ...
                    if field == "level":
                        ...
                        modality, rationale, evidence = _parse_modality_and_rationale(value)
