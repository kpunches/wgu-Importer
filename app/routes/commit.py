...
async def find_existing_comps(client, doc_id, course_row_id, headers):
    ...
    Returns dict of {normalized_title: row_id} for dedup.
    """
    resp = await client.get(
        f"{CODA_API_BASE}/docs/{doc_id}/tables/{GRID_COURSES_COMPS}/rows",
        headers=headers,
        params={"limit": 100, "valueFormat": "simple"},
    )
    if not resp.is_success:
        return {}
    existing = {}
    for row in resp.json().get("items", []):
        vals = row.get("values", {})
        # c-xObNqSoOEJ = Course LU (relation — value is display string or row ref)
        # c-REXJBHpf4F = Competency Title Display (formula column, safe to read)
        title = vals.get("c-REXJBHpf4F", "")
        course_ref = vals.get("c-xObNqSoOEJ", "")
        # Only include rows for this course
        if course_row_id in str(course_ref):
...
async def find_existing_rsds(client, doc_id, pcc_row_id, headers):
    ...
    Returns dict of {normalized_title: rsd_row_id} for dedup.
    """
    resp = await client.get(
