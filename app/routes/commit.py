...
async def find_existing_comps(client, doc_id, course_row_id, headers, course_code="", course_name=""):
    """
    Read _Courses_Comps for rows linked to this course.
    Coda returns relation columns as display strings (e.g. "D969: Legal and Ethical..."),
    so we match on course_code prefix or course_name substring instead of row ID.
    Returns dict of {normalized_comp_title: cc_row_id} for dedup.
    """
    ...
    existing = {}
    # Build match strings — course display is "CODE: Name" or just "Name"
    match_prefix = f"{course_code}:" if course_code else ""
    match_name = normalize(course_name)

    for row in resp.json().get("items", []):
        vals = row.get("values", {})
        # c-xObNqSoOEJ = Course LU (returns display string like "D969: Legal and Ethical...")
        course_display = str(vals.get("c-xObNqSoOEJ", ""))
        norm_display = normalize(course_display)

        # Match by code prefix OR normalized name
        is_match = (
            (match_prefix and course_display.startswith(match_prefix)) or
            (match_name and match_name in norm_display)
        )
        if not is_match:
            continue

        # c-REXJBHpf4F = Competency Title Display (formula column)
        title = str(vals.get("c-REXJBHpf4F", ""))
        if title:
            existing[normalize(title)] = row["id"]
    return existing
...
async def find_existing_rsds(client, doc_id, pcc_row_id, headers):
    ...
    for row in resp.json().get("items", []):
        vals = row.get("values", {})
        # c-n8SfKhfiaW = PCC RSD Row ID (plain text formula — contains the PCC row ID)
        pcc_rsd_row_id = str(vals.get("c-n8SfKhfiaW", ""))

        # Also check the PCC relation display string contains our pcc_row_id
        pcc_ref_display = str(vals.get("c-5n6qQLcwC1", ""))

        if pcc_row_id not in pcc_rsd_row_id and pcc_row_id not in pcc_ref_display:
            continue

        # c-aMkASg0dfw = Skill Title (plain text)
        skill_title = str(vals.get("c-aMkASg0dfw", ""))
        if skill_title:
            existing[normalize(skill_title)] = row["id"]
    return existing
...
async def commit(req: CommitRequest):
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        existing_comps = await find_existing_comps(
            client, doc_id, course_row_id, headers,
