...
async def create_row(client, doc_id, grid_id, cells, headers, retries=6):
    ...
    for attempt in range(retries):
        ...
        if resp.status_code == 429:
            ...
            await asyncio.sleep(max(retry_after, 2.0 * (attempt + 1)))
            continue
    ...
    raise HTTPException(
        status_code=502,
        detail=f"Coda rate limit not resolved after {retries} retries [{grid_id}]"
    )
...
def rcw_action(coda_val, source_val):
    """
    Read-Compare-Write decision per coda-mcp-patterns canonical spec.
    Returns: 'CREATE' | 'SKIP' | 'DIFF'
    """
    c = normalize(coda_val)
    s = normalize(source_val)
    if not c and s:
        return "CREATE"
    elif c == s:
        return "SKIP"
...
async def find_existing_comps(client, doc_id, course_row_id, headers):
    """
    Read _Courses_Comps for all comps already linked to this course.
    Uses raw (non-simple) valueFormat so relation columns return row IDs.
    Returns dict of {normalized_title: cc_row_id} for dedup.
    """
    resp = await client.get(
        f"{CODA_API_BASE}/docs/{doc_id}/tables/{GRID_COURSES_COMPS}/rows",
        headers=headers,
        params={"limit": 200},  # raw format — relations include row IDs
    )
    if not resp.is_success:
        return {}
    existing = {}
    for row in resp.json().get("items", []):
        vals = row.get("values", {})
        # Raw format: relation columns return {"id": "i-...", "name": "..."}
        # c-xObNqSoOEJ = Course LU
        course_ref = vals.get("c-xObNqSoOEJ", {})
        course_ref_id = ""
        if isinstance(course_ref, dict):
            course_ref_id = course_ref.get("id", "")
        elif isinstance(course_ref, list) and course_ref:
            course_ref_id = course_ref[0].get("id", "") if isinstance(course_ref[0], dict) else str(course_ref[0])
        else:
            course_ref_id = str(course_ref)

        # DEBUG: log first row's course_ref structure so we can see the format
        if not existing:  # only log once
            import sys
            print(f"DEBUG course_ref type={type(course_ref).__name__} value={repr(course_ref)[:200]}", file=sys.stderr)
            print(f"DEBUG course_ref_id={repr(course_ref_id)} target={repr(course_row_id)}", file=sys.stderr)

        if course_ref_id != course_row_id:
            continue

        # c-REXJBHpf4F = Competency Title Display (formula, readable string)
        title = vals.get("c-REXJBHpf4F", "")
        if isinstance(title, dict):
            title = title.get("name", "") or title.get("content", "")
        existing[normalize(str(title))] = row["id"]
    return existing


async def find_existing_rsds(client, doc_id, pcc_row_id, headers):
    """
    Read _PCC_RSDs for all skills already linked to this PCC row.
    Returns dict of {normalized_title: junction_row_id} for dedup.
    """
    resp = await client.get(
        f"{CODA_API_BASE}/docs/{doc_id}/tables/{GRID_PCC_RSDS}/rows",
        headers=headers,
        params={"limit": 200},  # raw format for relation columns
    )
    if not resp.is_success:
        return {}
    existing = {}
    for row in resp.json().get("items", []):
        vals = row.get("values", {})
        # c-5n6qQLcwC1 = Program Course Competency (relation)
        pcc_ref = vals.get("c-5n6qQLcwC1", {})
        pcc_ref_id = ""
        if isinstance(pcc_ref, dict):
            pcc_ref_id = pcc_ref.get("id", "")
        elif isinstance(pcc_ref, list) and pcc_ref:
            pcc_ref_id = pcc_ref[0].get("id", "") if isinstance(pcc_ref[0], dict) else str(pcc_ref[0])
        else:
            pcc_ref_id = str(pcc_ref)

        if pcc_ref_id != pcc_row_id:
            continue

        # c-aMkASg0dfw = Skill Title (plain text)
        skill_title = vals.get("c-aMkASg0dfw", "")
        if isinstance(skill_title, dict):
            skill_title = skill_title.get("name", "") or skill_title.get("content", "")
        existing[normalize(str(skill_title))] = row["id"]
    return existing
...
async def commit(req: CommitRequest):
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        # ── Steps 4-9: Per-competency cascade (with RCW dedup) ─────────────────
        existing_comps = await find_existing_comps(client, doc_id, course_row_id, headers)
        log.append(f"  RCW: {len(existing_comps)} existing comp(s) found for this course")
        ...
        for comp in competencies:
            ...
            # Step 4: RCW — skip if comp already exists for this course
            if norm_title in existing_comps:
                comp_row_id = existing_comps[norm_title]
                log.append(f"  RCW Step 4: SKIP _Comps [{order}] '{title}' — exists ({comp_row_id})")
                pcc_row_id = None
            ...
            # Steps 7-8: Skills with RCW dedup
            existing_rsds = await find_existing_rsds(client, doc_id, pcc_row_id, headers)

            for skill in comp.get("skills", []):
                skill_title = skill.get("title", "")
                norm_skill = normalize(skill_title)

                if norm_skill in existing_rsds:
                    log.append(f"  RCW Step 7: SKIP RSD '{skill_title}' — already exists")
                    continue

                rsd_cells = [
                    {"column": COL_RSD_CATEGORY, "value": skill.get("category", "")},
                    {"column": COL_RSD_TITLE,     "value": skill_title},
                ...
                log.append(f"  ✓ Step 7: Created RSD '{skill_title}' -> {rsd_row_id}")
                written += 1
                await asyncio.sleep(2.0)
